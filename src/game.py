...
while not command.casefold() in ["q", "x"]:
    ...
    if command == "d" and player.can_move(1, 0, g):  # move right
        my_direction = [1,0]
    elif command == "w" and player.can_move(0, -1, g):  # move right
        my_direction = [0,1]
    elif command == "a" and player.can_move(-1, 0, g):  # move right
        my_direction = [-1,0]
    elif command == "s" and player.can_move(0, 1, g):  # move right
        my_direction = [0,-1]
